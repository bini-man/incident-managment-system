<!DOCTYPE html>
<html>

<head>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="css/style_sidebar.css">
</head>

<body>
  <div class="sidenav">
    <br>
    <a href=""><span>Admin Dashboard</span></a>
    <a href="incident.php" class="glyphicon glyphicon-book"> Incident Library </a>
    <a href="Create_incident.php" class="glyphicon glyphicon-plus"> Create Incident</a>
    <a href="Assign_incident.php" class="glyphicon glyphicon-share"> Assign Incident</a>
    <a href="Manage_user.php" class="glyphicon glyphicon-user"> Manage Users</a>
    <a href="Create_user.php" class="glyphicon glyphicon-plus"> Create Users</a>
    <a href="logout_admin.php" class="glyphicon glyphicon-log-out"> Logout</a>
  </div>
</body>

</html>